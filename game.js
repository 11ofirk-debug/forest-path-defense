...
async function loadLevelData() {
    const tabs = ["path", "slots", "enemies", "towers", "waves"];
...
const LUMBERJACK_ANCHOR_OFFSET = 8;  // tweak this per sprite
const CATAPULT_ANCHOR_OFFSET   = 12;  // tweak this per sprite
const WARDEN_ANCHOR_OFFSET = 8; // tweak as needed
const CROSSBOW_ANCHOR_OFFSET = 10; // tweak as needed
...
function drawEnemies() {
    for (let enemy of enemies) {
        ...
        const barW = sw;
        ctx.fillStyle = "red";
        ctx.fillRect(drawX, drawY - 6, barW, 4);
        ctx.fillStyle = isWarden ? "orange" : "lime";
        ctx.fillRect(drawX, drawY - 6, barW * (enemy.hp / enemy.maxHp), 4);
    }
}
    ...
    if (activeMenu.mode === "slot") {
        ...
        if (px >= menuX && px <= menuX + MENU_W && py >= menuY && py <= menuY + MENU_H_SLOT) {
            ...
            if (relY < (MENU_ROW1 + MENU_ROW2) / 2) {
                ...
                if (gold >= catDef.cost) {
                    towers.push({ type: "catapult", col: activeMenu.col, row: activeMenu.row,
               ...
               const bowDef = towerDefs["crossbow"];
                if (gold >= bowDef.cost) {
                    towers.push({ type: "crossbow", col: activeMenu.col, row: activeMenu.row,
        ...
        if (closest) {
            ...
            if (tower.type === "catapult") {
                ...
                for (let other of enemies) {
                    ...
                    if (d <= 50) {
                        other.hp -= tower.damage * 0.5;
                        if (other.hp <= 0) {
                            other.dead = true;
                            gold += other.type === "warden" ? 12 : 5;
                            updateUI();
                        }
                    }
                }
                // Spawn splash animation at impact point
                splashes.push({
                    x: closest.x,
                    y: closest.y,
                    maxRadius: 50,
            ...
            tower.target = null; // no more line drawing
